...
# Cleanup Merged Worktree (QuickCut)
...
Run all commands from the **main checkout** (`/Users/.../quickcut`),
...
```
User: I merged PR #125 and deleted the remote branch. Clean up.

You: I'm using the cleanup-merged-worktree skill to remove the merged
worktree and refresh main.

[git worktree list -> .worktrees/feature-issue-120-auto-mark-notifications]
[git -C ... status --porcelain -> empty]
[git -C ... log @{u}..HEAD -> empty]
[gh pr list --state merged --head feature-issue-120-auto-mark-notifications -> #125]
[git worktree remove .worktrees/feature-issue-120-auto-mark-notifications]
[git branch -d feature-issue-120-auto-mark-notifications -> deleted]
[git pull --ff-only origin main -> 4e6fb22..8d575db]
[git remote prune origin -> pruned origin/feature-issue-120-auto-mark-notifications]

Worktree removed.
Local branch deleted.
main: 4e6fb22 -> 8d575db (PR #125 merge).
